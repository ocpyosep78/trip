		...
		if (mysql_num_rows($query) == 0) {
	        ...
	        if (($provider == 'google' || $provider == 'microsoft' || $provider == 'linkedin') && ($loginProvider == 'facebook')) {
				...
				mysql_query( "
					UPDATE users
					SET
						gender='$gender',location = '$location',hometown = '$hometown',bio='$bio',relationship='$relationship',timezone='$timezone',
						provider='$loginProvider',provider_id='$provider_id',picture='$picture'
					WHERE id = '$id';
				");
		       	return $id;
			} else {
