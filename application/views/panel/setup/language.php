...
$(document).ready(function() {
	...
	var param = {
		id: 'datatable',
		source: web.base + 'panel/setup/language/grid',
		...
		callback: function() {
			$('#datatable .btn-edit').click(function() {
				...
				Func.ajax({ url: web.base + 'panel/setup/language/action', param: { action: 'get_by_id', id: record.id }, callback: function(result) {
					Func.populate({ cnt: '#modal-language', record: result });
			...
			$('#datatable .btn-delete').click(function() {
				...
				Func.confirm_delete({
					...
					url: web.base + 'panel/setup/language/action', callback: function() { dt.reload(); }
				});
			});
		}
	}
	...
	// form
	var form = $('#modal-language form').parsley();
	$('#modal-language [name="title"]').keyup(function() {
		var value = Func.GetName($(this).val());
		$('#modal-language [name="alias"]').val(value);
	});
	$('#modal-language form').submit(function(e) {
		e.preventDefault();
		if (! form.isValid()) {
			return false;
		}
		...
		Func.update({
			link: web.base + 'panel/setup/language/action',
			param: Site.Form.GetValue('modal-language form'),
			callback: function() {
				dt.reload();
	...
	$('.show-dialog').click(function() {
		$('#modal-language').modal();
		$('#modal-language form')[0].reset();
		$('#modal-language form').parsley().reset();
